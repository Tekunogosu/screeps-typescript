...
export const loop = ErrorMapper.wrapLoop(() => {
    ...
    if (CreepCount(RoleType.Builder) < 0) {
        console.log(Spawner(RoleType.Builder, [WORK, WORK, WORK, WORK, CARRY, CARRY, CARRY, MOVE], 
            CreepCount(RoleType.Harvester) >= 2 && CreepCount(RoleType.Upgrader) >= 2));
    }

    if (CreepCount(RoleType.Upgrader) < 0) {
        console.log(Spawner(RoleType.Upgrader, [WORK, WORK, WORK, WORK,CARRY, CARRY, MOVE, MOVE]), CreepCount(RoleType.Harvester) >= 3);
    }
    
    if (CreepCount(RoleType.Upgrader) < 1) {
        console.log(Spawner(RoleType.Upgrader, [WORK, WORK, CARRY, MOVE]), CreepCount(RoleType.Harvester) >= 2);
    }
    
    if (CreepCount(RoleType.Hauler) < 0) {
        console.log(Spawner(RoleType.Hauler, [CARRY, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE]), CreepCount(RoleType.Harvester) >= 2);
    }
